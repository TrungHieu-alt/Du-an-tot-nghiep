...
describe('http', () => {
  ...
  it('builds request URL with configured base URL and JSON body', async () => {
    ...
    await http<{ ok: boolean }>('/auth/register', {
      method: 'POST',
      body: { email: 'dev@example.com' },
    });

    expect(fetchMock).toHaveBeenCalledTimes(1);
    expect(fetchMock.mock.calls[0][0]).toBe(`${ENV.API_BASE_URL}/auth/register`);
  ...
  it('keeps default fallback message when backend returns only detail', async () => {
    const fetchMock = vi.fn().mockResolvedValue(
      new Response(JSON.stringify({ detail: 'Unauthorized' }), { status: 401 })
    );
    global.fetch = fetchMock as unknown as typeof fetch;

    await expect(http('/users/login', { method: 'POST', body: {} })).rejects.toThrow(
      'Đã có lỗi xảy ra'
    );
  });
