...
export default function ResumeCreatePage() {
  ...
  const [form, setForm] = useState({ title: "", location: "ha_noi", job_type: "fulltime", seniority: "mid", education: "dai_hoc", skills: "", summary: "", experience: "" });
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      const res = await resumesApi.create({ ...form, skills: form.skills.split(",").map(s => s.trim()).filter(Boolean) }, token);
      navigate(`/records/resumes/${res.resume_id}`, { replace: true });
    } catch (err) { setError(err instanceof ApiError ? err.body.message : "Lỗi tạo CV."); }
    finally { setSubmitting(false); }
  }

  return (
    <div className="flex h-full flex-col">
      <PageHeader title="Tạo CV mới" />
      <div className="flex-1 overflow-y-auto p-6">
        <form onSubmit={handleSubmit} className="max-w-lg space-y-4">
          <FormField label="Tiêu đề *"><input className={inputCls()} value={form.title} onChange={set("title")} required placeholder="vd. Backend Developer" /></FormField>
          ...
          <FormField label="Tóm tắt"><textarea className={inputCls()} rows={3} value={form.summary} onChange={set("summary")} /></FormField>
          <FormField label="Kinh nghiệm"><textarea className={inputCls()} rows={4} value={form.experience} onChange={set("experience")} /></FormField>
          {error && <p className="text-sm text-red-500">{error}</p>}
          <div className="flex gap-3">
            <button type="submit" disabled={submitting} className="rounded-md bg-slate-900 px-4 py-2 text-sm text-white hover:bg-slate-700 disabled:opacity-60">{submitting ? "Đang tạo..." : "Tạo CV"}</button>
