...
1. Slice 6B remains the compact scenario dataset (6 JD / 36 CV). Slice 6C broad ranking verification now runs on the default SQL seed path with 10 JD / 34 CV.
...
Out of scope:
- Production-scale 40 JD / 240 CV dataset.
...
This plan prepares the Matching V2 scenario dataset and verification flow. Slice 6C uses a broad SQL seed extension for 10 representative JD anchors; production-scale 40 JD / 240 CV data remains deferred.
...
| Slice 6B — Minimal Scenario JD/CV Dataset + Local Embeddings | Tạo dataset nhỏ nhất nhưng đủ kiểm tra các kịch bản matching V2 cốt lõi trước khi scale. Dataset có 6 JD + 36 CV, mỗi JD có đúng 6 CV được thiết kế quanh nó, không cộng dồn seed cũ. | Prompt/tool tạo JD/CV JSON; JSON scenario dataset; JSON schema validation; seed profile `scenario` hoặc reset script truncate rồi insert; local MiniLM embedding generator; scenario matrix mô tả pass/fail/ranking kỳ vọng; post-generation data validation report; docs cách seed scenario dataset. | 1. Scenario seed không cộng dồn seed cũ: dùng seed profile `scenario` hoặc script truncate rồi insert.<br>2. Sau scenario reset DB có đúng 6 JD + 36 CV, không cộng dồn seed demo cũ.<br>3. Mỗi JD có đúng 6 CV được thiết kế quanh nó.<br>4. Dataset phủ tối thiểu 6 nhóm: Backend/DevOps, Frontend, AI/Data, Product/BA, Sales/Marketing, Finance/HR/Admin hoặc Ops/Service.<br>5. Có đúng 6 `job_embeddings_v2` + 35 `candidate_embeddings_v2`, với CV `3018` thiếu embedding row có chủ ý và được ghi rõ trong scenario matrix.<br>6. Dataset chỉ có JD/CV/embeddings, không có user/recruiter/application.<br>7. JSON schema validate taxonomy: `location`, `job_type`, `seniority`, `education`.<br>8. Skills/certifications lowercase, trim, unique.<br>9. Mỗi JD có ít nhất 1 strong pass, 1 good pass, 1 noisy pass.<br>10. Toàn dataset cover đủ fail cases: sai location non-remote, sai job_type, sai seniority, education thấp, thiếu certification, skill overlap thấp, title giống skill khác, skill giống nhưng fail seniority/location.<br>11. Có case explicit cho JD `remote` bỏ qua location.<br>12. Có case explicit cho non-remote strict location.<br>13. Có case education hierarchy: cao hơn pass, thấp hơn fail.<br>14. Có case multi-cert subset fail.<br>15. Có case `total_after_filter = 0`.<br>16. Có case `min_score = 0` để tách hard-filter exclusion khỏi score threshold.<br>17. Có case tie-break deterministic theo ID.<br>18. Có case `top_k` khi số candidate pass filter > `top_k`.<br>19. Có missing embedding case: không crash, score field thiếu = 0, reasoning ghi rõ.<br>20. Mỗi CV khác nhau ít nhất 3 nhóm: title, skills, summary, experience, seniority, education, location/job_type, certifications.<br>21. Embedding generator mặc định dùng local MiniLM, normalized 384-dim, không network/API; fixture deterministic chỉ dùng cho tests qua `DB_V2_USE_DETERMINISTIC_FIXTURE_EMBEDDINGS=1`.<br>22. Không dùng vector hằng làm mất ý nghĩa cosine ranking.<br>23. Reset + seed chạy sạch trên DB trống.<br>24. Post-generation validation gate pass và được ghi lại bằng command/output summary.<br>25. Broad 40/240 dataset được defer sang later slice. |
| Slice 6C — Ranking Expectations + E2E Verification | Kiểm tra matching trên broad SQL seed dataset bằng expected ranking, tách rõ hard-filter exclusion khỏi `min_score` exclusion. | Ranking expectation JSON; tests cho JD -> CV và CV -> JD; hard-filter tests; smoke live; OpenAPI V2 namespace check. | 1. Expectations có `anchor_type`, `anchor_id`, `top_k`, `min_score`, `expected_top_id`, `must_include`, `must_exclude`, `must_exclude_reason`, `must_rank_above`.<br>2. Hard-filter exclusion test chạy với `min_score=0` hoặc kiểm tra `total_after_filter` để không nhầm với score threshold.<br>3. Strong match đứng trên good match trong expectation chính.<br>4. Good match đứng trên noisy pass trong expectation chính.<br>5. Fail hard-filter CV không xuất hiện trong matches vì filter, không phải vì `min_score`.<br>6. Có expected top cho 8-12 representative JD anchors, và thêm CV -> JD representative anchors.<br>7. Không hard-code exact `final_score` toàn dataset; nếu cần dùng score range.<br>8. JD -> CV endpoint chạy được trên broad SQL seed dataset.<br>9. CV -> JD endpoint chạy được trên broad SQL seed dataset.<br>10. Response có đủ score fields: `final_score`, `title_score`, `skills_score`, `req_exp_score`, `req_summary_score`, `reasoning`, và runtime metrics.<br>11. Ranking deterministic khi chạy lại cùng seed.<br>12. Tests pass.<br>13. OpenAPI check ghi rõ namespace V2 prototype có đúng 2 matching endpoint: `/api/v2/prototype/matching/job/{job_id}/run` và `/api/v2/prototype/matching/cv/{cv_id}/run`. |
...
| 6e. Extra pass for top_k | JD-6 -> CV-35 | hard filters pass, plausible score | Ensures pass pool is larger than `top_k` | Planned |
...
## Post-Generation Data Validation Gate

Slice 6B is not complete when files are generated. After generating JSON, embeddings, and seed output, run a validation gate and record command/output summary in the handoff. The gate must fail fast if any generated data is unstable, over-produced, under-produced, or no longer matches this scenario matrix.

Required validation checks:

| Check | Required evidence |
|---|---|
| Dataset counts | JSON and DB counts are exactly 6 JD and 36 CV after scenario reset. |
| Per-anchor grouping | Each JD has exactly 6 designed CV mapped in the scenario matrix. |
| Prototype scope | Dataset contains only JD/CV prototype rows and embeddings; no user/recruiter/candidate/application rows. |
| Taxonomy schema | `location`, `job_type`, `seniority`, and `education` values are valid against `docs/REQUIREMENTS.md`. |
| Normalization | All skills/certifications are lowercase, trimmed, and unique after validation. |
| Scenario coverage | Every row in Planned Scenario Matrix is represented by at least one concrete JD/CV ID or expectation entry. |
| Strong/good/noisy | Each JD has at least one strong pass, one good pass, and one noisy pass. |
| Isolated hard-filter failures | Location, job_type, seniority, education, and certification fail cases are isolated where the matrix says they are isolated. |
| CV diversity | Sibling CVs for the same JD differ in at least 3 groups: title, skills, summary, experience, seniority, education, location/job_type, certifications. |
| Embedding shape | Every non-missing embedding vector is 384-dimensional. |
| Embedding normalization | Every non-missing embedding vector has a norm close to 1.0. |
| Non-constant embeddings | Generated vectors are not all identical or same-direction constant vectors. |
| Deterministic embeddings | Running the generator twice on the same JSON produces byte-identical or numerically identical vectors within a tight tolerance. |
| Reset idempotency | Running scenario reset + seed twice yields the same counts and same deterministic IDs, without accumulating rows. |
| Missing/partial embedding intent | Any missing/partial embedding case is explicitly named in the scenario matrix and expected output. |
| Ranking smoke | At least one JD -> CV smoke confirms strong > good > noisy on generated data. |
| Hard-filter separation smoke | At least one `min_score=0` smoke confirms hard-filter exclusions remain absent while low-score pass candidates can appear. |

Recommended validation tooling:
- Prefer a script such as `validate_scenario_dataset.py` or equivalent test helper over manual spot checks.
- The script should validate JSON before DB insert and validate DB state after reset + seed.
- The script should print a compact summary: counts, coverage status, embedding status, deterministic hash, and failed checks if any.

Do not proceed to Slice 6C expectation work until this gate passes.
...
1. Scenario reset does not accumulate earlier seed data.
...
6. There are exactly 35 `candidate_embeddings_v2` rows, with CV `3018` intentionally missing its embedding row.
...
12. Post-generation data validation gate passes and records command/output summary.
13. Reset + seed runs cleanly on an empty DB.
14. Running reset + seed twice is idempotent and does not accumulate data.
15. Broad 40/240 dataset is explicitly deferred to a later slice.
