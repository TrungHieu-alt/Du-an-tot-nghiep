...
Archived static reference generated from the removed V2 prototype tooling.

Historical source: `backend/db_v2/scenario/dataset.json`.  
Historical seed script: `python -m db_v2.scenario.reset_scenario`.  
Historical validation: `python -m db_v2.scenario.validate` (15 checks).

The `backend/db_v2` runtime/tooling no longer exists in the repository. Keep
this matrix only as deterministic matching reference material.
...
Historically, embeddings were generated by `db_v2/scenario/embedder.py`:
