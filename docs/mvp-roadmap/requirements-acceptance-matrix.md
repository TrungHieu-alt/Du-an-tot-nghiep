...
| Recruiter profile, organization, and job lifecycle | `docs/REQUIREMENTS.md` sections 4.2, 6.1, 6.3 | 2, 3, 5, 12, 15, 16 | Recruiter flow integration test; publish/close smoke; organization ownership tests | partial | Organization/job ownership, partial PATCH, publish/close guards, upload-to-draft JD parse path, and Independent organization seed/bootstrap tooling are covered. Full recruiter E2E remains follow-up. |
...
| Candidate apply flow | `docs/REQUIREMENTS.md` section 4.3, 6.6 | 9, 10, 12, 15, 16 | Candidate apply integration test; duplicate application test; notification/audit check | accepted | Slice 9 verified apply success, inactive resume rejection, closed job rejection, duplicate `(job_id, resume_id)` 409, application event, notification, audit, and linked summary DTOs. Full E2E/frontend smoke remains Slice 12/15/16. |
| Recruiter invite flow | `docs/REQUIREMENTS.md` section 4.3, 6.7 | 9, 10, 12, 15, 16 | Invite create/accept/reject integration tests; duplicate accept behavior test | accepted | Slice 9 verified invite create, closed job rejection, accept creates application, accept returns existing application without duplicate application/event rows, and reject creates no application. Full E2E/frontend smoke remains Slice 12/15/16. |
| Application lifecycle | `docs/REQUIREMENTS.md` sections 4.3, 6.6 | 9, 10, 12, 15, 16 | Status transition unit tests; event history verification; terminal lock smoke | accepted | Slice 9 verified canonical transition graph, invalid transition denial, terminal lock, status-change side effects, and ordered application detail event history. |
| Notifications and email | `docs/REQUIREMENTS.md` sections 4.4, 6.8 | 10, 12, 15, 16 | Notification row checks; email failure non-rollback test; frontend notification smoke | accepted | Slice 10 verifies in-app notifications, email attempts, local/log adapter, SMTP provider path, and fake-provider failure non-rollback for application and parse failure flows. Frontend notification UI smoke remains later. |
| Audit logs | `docs/REQUIREMENTS.md` sections 6.9, 60 HLD | 10, 11, 12, 16 | Audit row checks for required events; admin audit filter smoke | accepted | Slice 10 verifies audit rows with actor, event type, target, timestamp, metadata, email failure audit, business lifecycle audit, and admin audit-log query smoke. Broader Slice 11 monitoring UX/filter coverage remains later. |
