...
| 7 Embedding And Semantic Search | done | AI agent | v2 (working tree) | 2026-05-16 | 2026-05-16 | dirty: integrations/embedding/*, router.py, worker.py, tests/test_embedding_provider.py, HLD §40, progress | none | Hand off to Slice 8 (Matching Production Hardening). | docker compose healthy; `python -m unittest discover -s tests` → 124/124 OK (21 new embedding tests); `GET /api/health` 200; `/openapi.json` 47 paths / 58 ops (no contract change). | LocalHashEmbeddingProvider returns 384-dim deterministic vectors with version `hash-v1`; OpenAIEmbeddingProvider calls `/v1/embeddings` with `dimensions=384`, version `openai-{model}-v1`; dimension-mismatch + HTTP errors raise EmbeddingError; worker maps EmbeddingError → `embedding_failed` parse status; missing-embedding scoring + reasoning regression covered; factory falls back to local without API key. | Env vars: `EMBEDDING_PROVIDER` (local\|openai, default local), `OPENAI_EMBEDDING_API_KEY` (fallback to `OPENAI_API_KEY`), `OPENAI_EMBEDDING_MODEL` (default text-embedding-3-small), `OPENAI_EMBEDDING_BASE_URL`, `OPENAI_EMBEDDING_TIMEOUT_SECONDS=30`. DB dim fixed at 384 — providers that cannot produce 384-dim vectors must not be registered. API/OpenAPI impact: `non-breaking`. |
