...
| Endpoint | Gap | Fix | Impact |
|---|---|---|---|
| `POST /api/applications` | Duplicate and invalid-state behavior needed command-backed coverage. | Candidate apply requires active owned resume + published non-closed job; duplicate `(job_id, resume_id)` returns `409 duplicate_application`; success writes application event, notification, and audit. | non-breaking |
| `POST /api/applications/{id}/status` | Full transition graph and terminal lock needed hardening evidence. | Canonical graph and terminal-state lock are enforced; valid status changes write application event, notification, and audit. | non-breaking |
| `POST /api/invites` | Invite validation needed closed-job and side-effect coverage. | Recruiter invite requires active resume + owned published non-closed job; duplicate pending invite returns `409 duplicate_invite`; success writes notification and audit without creating an application. | non-breaking |
| `POST /api/invites/{id}/accept` | Duplicate accepted-invite behavior needed verification. | Accept creates an application with application event/notification/audit, or returns the existing `(job_id, resume_id)` application without duplicate application/event rows; invite acceptance writes notification and audit. | non-breaking |
| `POST /api/invites/{id}/reject` | Rejected invite no-application behavior needed verification. | Reject sets invite rejected, writes notification and audit, and creates no application or application event. | non-breaking |
